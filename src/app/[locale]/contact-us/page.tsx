...
export default function ContactPage() {
 ...
 const branchKey = (slug: string) => `branches.${slug}`;
  ...
  return (
    <div dir={isRTL ? "rtl" : "ltr"} className="min-h-screen bg-white pt-28 md:pt-32 pb-20">
            ...
            {contactLocations.map((location) => (
              <SwiperSlide key={location.id} className="!w-auto">
                ...
                >
                  ...
                  >
                 {t(`${branchKey(location.slug)}.name`)}
                  </h4>
                  <p className="text-xs text-gray-500 leading-tight group-hover:text-gray-600">
                   {t(`${branchKey(location.slug)}.community`)}, {t(`${branchKey(location.slug)}.city`)}
                  </p>
                </button>
              </SwiperSlide>
            ))}
          ...
          <div className="lg:w-5/12 order-2 lg:order-1">
            <div className="bg-white rounded-xl">
              ...
              <h2 className="text-3xl font-normal text-[#333333] mb-4">
                 {t(`${branchKey(selectedLocation.slug)}.name`)}
              </h2>

              <div className="flex items-start gap-3 text-gray-600">
                <MapPin size={18} className="mt-1 text-secondary shrink-0" />
                <p className="text-sm leading-relaxed break-words">
                {(t(`${branchKey(selectedLocation.slug)}.offAddress`) || "")
                  .split(", ")
