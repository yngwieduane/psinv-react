...
import { rakSliderData } from '@/data/homeSliderData';
import { getRAKCitiesData } from '@/utils/citiesDataRAK';
import ListPropertyForm from '../_components/ListPropertyForm';
import WhyPSI from '../_components/WhyPSI';
import CitiesTabRak from './components/CitiesTabRak';
...
const ReportDownload = dynamic(() => import('../_components/ReportDownloadSection'));

const montserrat = Montserrat({ subsets: ['latin'], display: 'swap', });
const outfit = Outfit({ subsets: ['latin'], display: 'swap', })

export default function RakHomePageClient() {
  ...
  const mainSliderData = rakSliderData.map((item) => {
    return {
      ...item,
  ...
  const citiesData = getRAKCitiesData(t_cities);
  ...
  return (
    <div>
      ...
      <section className={`relative min-h-[600px] ${outfit.className}`}>
        <HomeBanner slidePerView="1" cityFocus="rak" />
        <HomeSearch />
      </section>

      <section className="bg-gray-50">
        {/* <CitiesTab  cities={citiesData}  /> */}
        <CitiesTabRak cities={citiesData} />
      </section>
