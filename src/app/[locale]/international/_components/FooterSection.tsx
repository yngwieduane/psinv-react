...
function FooterSection() {
    const t = useTranslations('InternationalPage.footer_section');
                    ...
                    <div className="w-full md:flex lg:flex-row grid md:gap-8 md:px-5 px-6">
                        <div className="lg:w-1/12 md:w-1/10 w-full">
                            ...
                            before:h-[250px] before:bg-[#000] before:left-[15px]">
                                ...
                                inline-block ${montserratLight.className}' style={{ writingMode: "vertical-rl" }}>{t('vertical_text')}</div>
                                </h3>
                            </div>
                        </div>
                        <div className={`lg:w-1/4 md:w-9/10 w-full  ${montserratLight.className} md:space-y-10 space-y-5`}>
                            ...
                            <p className="md:text-2xl text-lg">{t('sub_title')}</p>
                            <div className="space-y-7">
                                ...
                                <div>
                                    <p className="md:text-lg text-md md:mb-5 mb-3 hover:text-[#ED9C4B] flex gap-4 items-start leading-none"><FontAwesomeIcon icon={faMapMarkerAlt} className="text-xl hover:text-[#ED9C4B]" />
                                        {t('locations.address1')}</p>
                                    <p className="md:text-lg text-md hover:text-[#ED9C4B] flex gap-4 items-start leading-none"><FontAwesomeIcon icon={faMapMarkerAlt} className="text-xl hover:text-[#ED9C4B]" />{t('locations.address2')}</p>
                                </div>
                            </div>
                            <div className="w-full">
                                <h2 className={`md:text-lg text-xl uppercase md:mb-4 mb-7 ${AudreyNormal.className} text-[#999] leading-tight`}>
                                    {t('social_media_title').split(' ').slice(0, 4).join(' ')}<br />{t('social_media_title').split(' ').slice(4).join(' ')}
                                </h2>
