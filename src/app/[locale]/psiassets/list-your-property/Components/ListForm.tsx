...
import { insertAssetsLead, insertHubspotLead } from "@/utils/crmApiHelpers";
...
const CITY_CONFIG: Record<string, { email: string; apiUrl: string; referredTo?: string; referredBy?: string; assignedTo?: string }> = {
    'Dubai': {
    ...
    'Abu Dhabi': {
        email: 'callcenter@psiassets.com',
    ...
    const onSubmit = async (data: FormData) => {
        ...
        try {
            ...
            if (uploadedFiles.propertyimages?.length > 0) {
                uploadedFiles.propertyimages.forEach((file: any) => {
                    filesToSend.push({
            ...
            if (uploadedFiles.spa?.length > 0) {
                filesToSend.push({
                    filename: uploadedFiles.spa[0].filename,
                    filedata: uploadedFiles.spa[0].filedata
                });
            }
            if (uploadedFiles.deed?.length > 0) {
                filesToSend.push({
                    filename: uploadedFiles.deed[0].filename,
                    filedata: uploadedFiles.deed[0].filedata
                });
            }
            if (uploadedFiles.passport?.length > 0) {
                filesToSend.push({
                    filename: uploadedFiles.passport[0].filename,
                    filedata: uploadedFiles.passport[0].filedata
                });
            }
            ...
            const remarks = `
                ...
                ${filesToSend.length > 0 ? `</br>Attachments: </br>${filesToSend.join('</br>')}` : ''}
            ...
            try {
                if (isHubspotMedia && !['Dubai', 'Sharjah'].includes(cityName)) {
                    ...
                    const hubspotResponse = await insertAssetsLead(formDataToSend);
