...
const UltraLuxuryProjectsSlider = ({ data, slides, onAction }: SliderProps) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        const currentSlide = slides[activeThumb];
        // Try to find matching project from the data pool to get a real ID
        const matchingProject = data?.find(p =>
          (p.propertyName || p.name)?.toLowerCase().includes(currentSlide.proj_name.toLowerCase()) ||
          currentSlide.proj_name.toLowerCase().includes((p.propertyName || p.name)?.toLowerCase())
        );

        const propertyId = matchingProject?.propertyID || matchingProject?.id || currentSlide.proj_name;

        const res = await fetch(`/api/external/unitsAssets?propertyId=${encodeURIComponent(propertyId)}`);

        if (!res.ok) {
          // Silent warning since units are not currently displayed in this slider
          console.warn(`Units fetch paused for ${currentSlide.proj_name} (Status: ${res.status})`);
          return;
        }
        ...
        const finalRes = Array.isArray(results) ? results.slice(0, 5) : [];
        // Note: results are currently not stored in state as they are not used in UI yet
      } catch (error) {
