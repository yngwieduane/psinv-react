...
export default function LuxuryHeader({ onAction, pageURL }: Props) {
    ...
    const navItems = [
        { name: t('Hero.nav.home'), url: "#home" },
        { name: t('Hero.nav.luxuryProjects'), url: "#luxury-projects" },
        { name: t('Hero.nav.ultraLuxuryProjects'), url: "#ultra-luxury-projects" },
        { name: t('Hero.nav.entertainment'), url: "#entertainment" },
        { name: t('Hero.nav.futureResidence'), url: "#future-residence" },
        { name: t('Hero.nav.vipTour'), url: "#vip-tours" },
    ];
        ...
        <div className={`relative w-full md:h-[750px] h-[450px] md:py-0 ${montserrat.className} bg-cover! bg-center! bg-no-repeat!`} id="home"
                    ...
                    <div>
                        ...
                        <ul className="flex flex-col gap-4 py-4 text-md font-semibold text-[#272963] leading-loose mt-5">
                            {navItems.map((item, index) => (
                                <a key={index} href={item.url} onClick={() => setIsMobileMenuOpen(false)}>
                                    <li key={index}>
                                        {item.name}
                                    </li>
                                </a>
                        ...
                        <button className="cursor-pointer btn text-white text-center bg-[#C19A5B] rounded-[5px] py-0 h-[40px] w-[175px]"
                            onClick={() => handleClick(t('Hero.nav.freeConsultation'))}>
                            {t('Hero.nav.freeConsultation')}
                        </button>
                    </div>
                </div>
            ...
            <div className="mx-auto md:flex max-w-full text-white px-4 pt-30">
                <div className="mx-auto text-center">
                    <h1 className="md:text-3xl text-3xl relative text-center">{t('Hero.title')}
                        <span className={`${parisienne.className} text-[#F2DAB4] md:text-8xl text-4xl`}> {t('Hero.luxury')}</span>
                        <span className="absolute -bottom-7 right-25"> {t('Hero.lifestyle')}</span>
                    </h1>
                </div>
            </div>
