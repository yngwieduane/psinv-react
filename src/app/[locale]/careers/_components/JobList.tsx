...
export default function JobList() {
  const [jobs, setJobs] = useState<Job[]>([]); // ✅ No duplicate
  ...
  return (
    ...
    >
      ...
      {/* Content Layer */}
      <div className={`relative flex flex-col md:flex-row items-center max-w-[1320px] mx-auto gap-12 transition-all duration-700 ${isHovered ? 'justify-between' : 'justify-center'} z-10`}>
        {/* Left PSI Logo */}
        <div className={`w-full md:w-1/2 flex transition-all duration-700 ${isHovered ? 'justify-start' : 'justify-center'}`}>
          <Image
            src="/images/career/PSI-Logo.svg"
            alt="PSI Logo"
            width={470}
        ...
        {/* Right Job List */}
        <div className={`w-full md:w-1/2 transition-all duration-700 ${isHovered ? 'opacity-100 max-w-full pl-5' : 'opacity-0 max-w-0 overflow-hidden'}`}>
          <Swiper
            direction="vertical"
            slidesPerView={5}
            spaceBetween={20}
            ...
            {jobs.map((job, index) => (
              <SwiperSlide key={job.id}>
                <a
                  ...
                  className={`block text-center border-2 py-4 rounded-md font-semibold transition mx-auto ${
                    activeIndex === index
