export function generateSeoData(pData: any) {
  ...
  if (!isNaN(pData.bedrooms) && Number(pData.bedrooms) > 0) {
    ...
    seoUrl = sanitizeTitle(`${pData.bedrooms} bedroom ${pData.propertyType} for ${pData.adType} ${pData.name} ${pData.community} ${pData.source} ${pData.code}`);
    ...
    // If it's a studio
    seoUrl = sanitizeTitle(`studio ${pData.propertyType} for ${pData.adType} ${pData.name} ${pData.community} ${pData.source} ${pData.code}`);
    ...
    seoUrl = sanitizeTitle(`${pData.propertyType} for ${pData.adType} ${pData.name} ${pData.community} ${pData.source} ${pData.code}`);
