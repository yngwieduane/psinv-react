...
const PropertyCard = (props:any) => {
  ...
  const propBed = props.data["availableBedrooms"] ? (<p className="text-sm">Beds<br />
    {props.data["availableBedrooms"].map(img => {
  ...
  const url = '/projects/' + slugify(props.data['country']) + "/" + slugify(props.data['city']) + "/" + slugify(props.data['community']) + "/" + slugify(subCommunity) + "/" + slugify(props.data['propertyName']);
  return (
    <Link href={url} className="h-full">
      <div className="w-96 group relative shadow-lg rounded-lg h-full">
        <div className="aspect-h-2 aspect-w-4 overflow-hidden bg-gray-100 relative rounded-lg">
          ...
          />
          <div
            className="flex items-center justify-center opacity-0 group-hover:opacity-100 ease-in duration-300"
            aria-hidden="true"
          >
